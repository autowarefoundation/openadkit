...
if [ "$mode" = "final" ]; then
    ...
    find /usr/lib/python3/dist-packages -maxdepth 1 \( -name 'setuptools-*.egg-info' -o -name 'setuptools-*.dist-info' -o -name 'pkg_resources-*.egg-info' -o -name 'pkg_resources-*.dist-info' \) -exec rm -rf {} + 2>/dev/null || true
fi
