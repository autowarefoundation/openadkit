...
The visualizer binds websockify to loopback (`127.0.0.1:6080`) by default, so `--network host` is required for standalone runs. Under host networking, `-p` is a no-op and the browser reaches noVNC at `https://localhost:6080/vnc.html`. For bridge networking, set `WEBSOCKIFY_BIND=0.0.0.0` and use `-p 127.0.0.1:6080:6080` to preserve host-only access.
...
| `WEBSOCKIFY_BIND` | `127.0.0.1` | IP address | Address websockify binds to. Under bridge networking, set it to `0.0.0.0` inside the container while publishing the host port on `127.0.0.1` |
