...
`test/run_self_test.sh` resolves an `ADMIT_TOOL_IMAGE` containing `manifest_admit` (with the fixture spec manifest baked in, so the QoS cases are enforced), builds a matrix of fixture images from the manifests under `test/fixtures/manifests/`, and asserts `deploy_check.sh`'s exit code on the compose sets under `test/fixtures/compose/`. It also runs the spec-manifest-less base tool image directly (not through `deploy_check.sh`) to assert that `admit-tool-entrypoint.sh` itself warns on stderr when it finds no spec manifest to pass:
...
| `compose.qos-reject.yaml`     | a lone provider offering `best_effort` where the fixture spec manifest declares `reliable`                                                                                                              | `1` (QOS_SPEC_MISMATCH — asserted on the verdict text, not just the exit code) |
| `compose.qos-conformant.yaml` | the same provider with exactly the QoS its spec declares — the A/B twin of `compose.qos-reject.yaml`, differing only in `reliability`                                                                   | `0`                                                                            |
...
The GitHub Actions workflow `deploy-admission-self-test.yaml` runs the self-test on `pull_request` (paths under `deployments/common/admission/**`) and on `workflow_dispatch` (with `core-repo` / `core-ref` inputs). It is **advisory (non-required)**: it becomes meaningful once `autoware_component_interface_admission` merges into `autoware_core` — the landing order is admission package first, then this gate.
