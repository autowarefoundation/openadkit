...
CYAN='\033[0;36m'
NC='\033[0m' # No Color
ZENOH_ENV_FILE="$SCRIPT_DIR/config.env"
...
run_compose() {
    ...
    case "$cmd" in
        "up")
            ...
            validate_args "${args[@]}"
            docker compose --env-file "$ZENOH_ENV_FILE" up "${args[@]}" "${services[@]}"
            ;;

        "down")
            echo -e "${RED}[${context_name}]${NC} Stopping and removing services (with volumes)..."
            docker compose --env-file "$ZENOH_ENV_FILE" stop "${services[@]}"
            docker compose --env-file "$ZENOH_ENV_FILE" rm -f -v "${services[@]}"
            ;;
        ...
        "dry-run")
            ...
            docker compose --env-file "$ZENOH_ENV_FILE" config -q
            ;;
        ...
        *)
            ...
            docker compose --env-file "$ZENOH_ENV_FILE" "$cmd" "${args[@]}" "${services[@]}"
            ;;
    esac
}
