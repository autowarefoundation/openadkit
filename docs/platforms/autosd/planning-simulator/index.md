...
The planning container launches `planning_simulator.launch.xml` with the
extracted map mounted at `/etc/awf/map` and RViz disabled (`rviz:=false`).

The simulator container runs `scenario_test_runner` once against the bundled
`sample.yaml` scenario (`global_timeout:=180` after `initialize_duration:=90`,
so allow a few minutes). Because it is a `Type=oneshot` service,
`systemctl status awf-oak-simulator.service` shows `inactive (dead)` after the
run — expected, not a failure. The planning container stays running.
