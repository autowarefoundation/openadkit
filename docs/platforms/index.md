...
Tiers say how seriously Open AD Kit treats a host or platform path. A path that
fails its build or validation gate is dropped or marked below, not documented as
if it worked. Hardware rows use the badges on the [hardware page](hardware/index.md).
...
| Component images amd64 + arm64 (except `carla-interface`, amd64 only) | **Committed** | Published per [image inventory](https://github.com/autowarefoundation/openadkit/blob/main/.github/image-inventory.json) |
